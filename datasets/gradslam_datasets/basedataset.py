...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if 'perturb_type' in config_dict.keys():
            self.perturb_type = config_dict['perturb_type'] #17
            self.perturb_severity=config_dict['perturb_severity']
            self.perturb_dynamic=config_dict['perturb_dynamic']

        if 'force_perturb' in config_dict.keys():
            self.force_perturb = config_dict['force_perturb']
        else:
            self.force_perturb = False
        self.start = start
        self.end = end
        if start < 0:
        ...
        if int(config_dict['frame_downsample']) >=2:
            stride = config_dict['frame_downsample']
            print(f"Enable fast motion and the speep-up ratio is {stride}")
        else:
            stride = 1
        ...
        if stride != 999:
            ...
            original_color_paths = self.color_paths 
            # Define a function for dynamic stride sampling
            def dynamic_stride_sampling(paths, start, end):
                sampled_paths = []
                stride = 1
                while start < end:
                    sampled_paths.append(paths[start])
                    start += stride
                    stride *= 2
                    if stride == 16:
                        stride = 1
            ...
            # Apply dynamic stride sampling to each path list
            self.color_paths = dynamic_stride_sampling(self.color_paths, self.start, self.end)
            self.depth_paths = dynamic_stride_sampling(self.depth_paths, self.start, self.end)
            if self.load_embeddings:
                self.embedding_paths = dynamic_stride_sampling(self.embedding_paths, self.start, self.end)
            self.poses = dynamic_stride_sampling(self.poses, self.start, self.end)

            # Calculate retained indices
            self.retained_inds = torch.tensor([original_color_paths.index(path) for path in self.color_paths])
        
            # Update self.num_images after subsampling the dataset
            self.num_imgs = len(self.color_paths)

            print(f"Sampled {self.num_imgs} frames and poses with recursive stride.")
            # add the code for dynamic stride here
    ...
    def brightness_perturber(self, image, brightness_factor):
    ...
    def __getitem__(self, index):
        ...
        if self.force_perturb == 1 and index >= (self.num_imgs//2):
            color = self.brightness_perturber(np.array(color), 0.3)
        elif self.force_perturb == 2:
            pass
        ...
        if perturb_type == 0:
            color = brightness(color,perturb_severity)
        elif perturb_type == 1:
            color = contrast(color,perturb_severity)
        elif perturb_type == 2:
            color = spatter(color,perturb_severity)
        elif perturb_type == 3:
            color = zoom_blur(color,perturb_severity)
        elif perturb_type == 4:
            color = motion_blur(color,perturb_severity)
        elif perturb_type == 5:
            color = defocus_blur(color,perturb_severity)
        elif perturb_type == 6:
            color = gaussian_noise(color,perturb_severity)
        elif perturb_type == 7:
            color = shot_noise(color,perturb_severity)
        elif perturb_type == 8:
            color = impulse_noise(color,perturb_severity)
        elif perturb_type == 9:
            color = speckle_noise(color,perturb_severity)
        elif perturb_type == 10:
            color = gaussian_blur(color,perturb_severity)
        elif perturb_type == 11:
            color = glass_blur(color,perturb_severity)
        elif perturb_type == 12:
            color = fog(color,perturb_severity)
        elif perturb_type == 13:
            color = frost(color,perturb_severity)
        elif perturb_type == 14:
            color = snow(color,perturb_severity)
        elif perturb_type == 15:
            color = jpeg_compression(color,perturb_severity)
        elif perturb_type == 16:
            color = pixelate(color,perturb_severity)
        elif perturb_type == 17:
            pass

        color = np.array(color)+0.0

        org_color = self._preprocess_color(org_color)
        ...
        if self.distortion is not None:
            org_color = cv2.undistort(org_color, K, self.distortion)
            color = cv2.undistort(color, K, self.distortion)

        org_color = torch.from_numpy(org_color)
        ...
        if perturb_type == 20:
            depth = depth_add_gaussian_noise(depth, perturb_severity)
        elif perturb_type == 21:
            depth = depth_add_edge_erosion(depth, perturb_severity)
        elif perturb_type == 22:
            depth = depth_add_random_mask(depth, perturb_severity)
        elif perturb_type == 23:
            depth = depth_add_fixed_mask(depth, perturb_severity)
        elif perturb_type >= 25:
            pass

        depth = self._preprocess_depth(depth)
        if perturb_type == 24:
            depth = depth_range(depth,perturb_severity)
        ...
        if self.force_perturb == False:
            ...
            return (
                ...
                intrinsics.to(self.device).type(self.dtype),
                pose.to(self.device).type(self.dtype),
                org_color.to(self.device).type(self.dtype)
                # self.retained_inds[index].item(),
               )
