...
def transform_to_frame(config, params, time_idx, gaussians_grad, camera_grad):
    """
    ...
    rel_w2c = torch.eye(4).to(config['primary_device']).float()
    ...
    pts_ones = torch.ones(pts.shape[0], 1).to(config['primary_device']).float()
