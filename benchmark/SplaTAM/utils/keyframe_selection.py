...
def get_pointcloud(config, depth, intrinsics, w2c, sampled_indices):
    CX = intrinsics[0][2]
    CY = intrinsics[1][2]
    FX = intrinsics[0][0]
    FY = intrinsics[1][1]
    ...
    B = torch.zeros((1, 3)).to(config['primary_device']).float()
...
def keyframe_selection_overlap(config, gt_depth, w2c, intrinsics, keyframe_list, k, pixels=1600):
        """
        ...
        pts = get_pointcloud(config, gt_depth, intrinsics, w2c, sampled_indices)
