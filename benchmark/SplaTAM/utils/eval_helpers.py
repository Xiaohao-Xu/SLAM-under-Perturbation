...
def calculate_relative_pose_error_rotation(gt_traj, est_traj):
    """
    Calculates the relative pose error for rotation between the ground truth trajectory and the estimated trajectory.

    Args:
        gt_traj (list): List of 4x4 matrices representing the ground truth trajectory.
        est_traj (list): List of 4x4 matrices representing the estimated trajectory.
            len(gt_traj) == len(est_traj)

    Returns:
        float: The average rotation error between the ground truth and estimated trajectories.

    Note:
        This function assumes that the rotation matrices are in the upper-left 3x3 submatrix of each 4x4 matrix.
        The rotation error is calculated using the formula: rotation_error = arccos((trace(R_gt^T * R_est) - 1) / 2),
        where R_gt and R_est are the ground truth and estimated rotation matrices, respectively.
    """
    gt_traj_rot = [gt_traj[idx][:3, :3] for idx in range(len(gt_traj))]
    est_traj_rot = [est_traj[idx][:3, :3] for idx in range(len(est_traj))]

    gt_traj_rot = torch.stack(gt_traj_rot).detach().cpu().numpy()
    est_traj_rot = torch.stack(est_traj_rot).detach().cpu().numpy()

    # Calculate the rotation error
    rotation_errors = []
    for i in range(len(gt_traj_rot)):
        gt_rot = gt_traj_rot[i]
        est_rot = est_traj_rot[i]
        rot_error = np.arccos((np.trace(np.dot(gt_rot.T, est_rot)) - 1) / 2)
...
def calculate_relative_pose_error_translation(gt_traj, est_traj):
    """
    Calculates the relative pose error for translation between the ground truth trajectory and the estimated trajectory.
    ...
    Returns:
        float: The average translation error between the ground truth and estimated trajectories.

    Note:
        This function assumes that the translation vectors are in the last column of each 4x4 matrix.
        The translation error is calculated as the Euclidean distance between the corresponding translation vectors.
    """
    gt_traj_trans = [gt_traj[idx][:3, 3] for idx in range(len(gt_traj))]
    est_traj_trans = [est_traj[idx][:3, 3] for idx in range(len(est_traj))]

    gt_traj_trans = torch.stack(gt_traj_trans).detach().cpu().numpy()
    est_traj_trans = torch.stack(est_traj_trans).detach().cpu().numpy()

    # Calculate the translation error
    translation_errors = np.linalg.norm(gt_traj_trans - est_traj_trans, axis=1)

    avg_trans_error = np.mean(translation_errors)

    return avg_trans_error

def calculate_relative_pose_error(gt_traj, est_traj):
    """
    Calculates the relative pose error between the ground truth trajectory and the estimated trajectory.
    ...
    Returns:
        float: The average pose error between the ground truth and estimated trajectories.

    Note:
        The pose error is calculated as the Frobenius norm of the difference between the corresponding transformation matrices.
    """
    gt_traj = torch.stack(gt_traj).detach().cpu().numpy()
    est_traj = torch.stack(est_traj).detach().cpu().numpy()

    # Calculate the pose error
    pose_errors = np.linalg.norm(gt_traj - est_traj, axis=(1, 2))
...
def report_progress(config, params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
                    tracking=False, mapping=False, wandb_run=None, wandb_step=None, wandb_save_qual=False, online_time_idx=None,
                    global_logging=True,eval_dir=None):
    if i % every_i == 0 or i == 1:
        ...
        if tracking:
            ...
            rpe_trans = calculate_relative_pose_error_translation(gt_w2c_list, latest_est_w2c_list)
            rpe_trans = np.round(rpe_trans, decimals=6)
            rpe_rot = calculate_relative_pose_error_rotation(gt_w2c_list, latest_est_w2c_list)
            rpe_rot = np.round(rpe_rot, decimals=6)
            rpe_all = calculate_relative_pose_error(gt_w2c_list, latest_est_w2c_list)
            rpe_all = np.round(rpe_all, decimals=6)
            if wandb_run is not None:
                tracking_log = {f"{stage}/Latest Pose Error":iter_pt_error, 
                               ...
                               f"{stage}/RPE TRANS":rpe_trans,
                               f"{stage}/RPE ROT":rpe_rot,
                               f"{stage}/RPE ALL":rpe_all}

            if not (tracking or mapping):
                traj_filename = eval_dir + os.sep + "traj_"+str(iter_time_idx)+".npz"  # Filename to save the trajectories
            elif tracking:
                traj_filename = eval_dir + os.sep + "traj_"+str(iter_time_idx)+".npz" 
            elif mapping:
                traj_filename = eval_dir + os.sep + "traj_"+str(online_time_idx)+".npz" 

            save_trajectory(gt_w2c_list, latest_est_w2c_list, traj_filename)
        ...
        if not (tracking or mapping):
            ...
            #progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | Rel Pose Error: {rel_pt_error.item():.{7}} | Pose Error: {iter_pt_error.item():.{7}} | ATE RMSE": f"{ate_rmse.item():.{7}}" "| RPE ROT": f"{rpe_rot.item():.{7}} | RPE TRANS": f"{rpe_trans.item():.{7}} | RPE ALL": f"{rpe_all.item():.{7}} "})
            progress_bar.set_postfix({
                    f"Time-Step: {iter_time_idx}": f"Rel Pose Error: {rel_pt_error.item():.{7}}",
                    "Pose Error": f"{iter_pt_error.item():.{7}}",
                    "ATE RMSE": f"{ate_rmse.item():.{7}}",
                    "RPE ROT": f"{rpe_rot.item():.{7}}",
                    "RPE TRANS": f"{rpe_trans.item():.{7}}",
                    "RPE ALL": f"{rpe_all.item():.{7}}"
                })
            progress_bar.update(every_i)
        elif mapping:
...
def save_trajectory(gt_traj, est_traj, filename):
    """
    Saves the ground truth trajectory (gt_traj) and estimated trajectory (est_traj) as NumPy arrays.

    Args:
        gt_traj (list): List of 4x4 matrices representing the ground truth trajectory.
        est_traj (list): List of 4x4 matrices representing the estimated trajectory.
        filename (str): The filename (including the path) to save the trajectories.

    Note:
        The ground truth and estimated trajectories should have the same length.
    """
    gt_traj_array = np.stack(gt_traj)
    est_traj_array = np.stack(est_traj)

    np.savez(filename, gt_traj=gt_traj_array, est_traj=est_traj_array)
...
def eval(config, dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Get current frame Gaussians
        transformed_pts = transform_to_frame(config, final_params, time_idx, 
                                             gaussians_grad=False,
                                             camera_grad=False)
