...
void System::keyFrameTimestampNumFeatures(const string &timestampFile) {
    cout << "Saving keyFrameTimestampNumFeatures to " << timestampFile << endl;
    ...
    for (auto const & pKF : vpKFs) {
        ...
        totalKP += pKF->N;
        totalMP += pKF->GetNumberMPs();
        count++;
    }

    f1 << totalKP / count << endl;
    f1 << totalMP / count << endl;
    f1.close();
    cout << "Average key points: " << totalKP / count << endl;
    cout << "Average map points: " << totalMP / count << endl;
}
