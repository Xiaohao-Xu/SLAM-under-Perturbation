...
EXPANDED_SCRIPT_NAME="run_replica_goslam_mono_faster_motion_disable_loop_closure_expanded.sh" # The name of the generated script
...
for sc in $SCENES; do
  for sev in 5 3 1; do
    for ds in 2 4 8 16; do
      EXPNAME="${sc}_${PT}_${sev}_${PD}_${ds}_disable_lc"
      ...
      echo "python run.py $CONFIG_PATH --device cuda:0 --mode $MODE --output \"\${OUT_DIR}/$sc/$EXPNAME\" --perturb_type $PT --perturb_severity $sev --frame_downsample $ds --perturb_dynamic $PD --enable_loop_closure False" >> $EXPANDED_SCRIPT_NAME
